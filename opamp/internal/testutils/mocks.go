...
// UpdateAgent implements services.AgentService
func (m *MockAgentService) UpdateAgent(ctx context.Context, agent *services.Agent) error {
	m.mu.Lock()
	defer m.mu.Unlock()
...
// UpdateGroup implements services.AgentService
func (m *MockAgentService) UpdateGroup(ctx context.Context, group *services.Group) error {
	...
	if _, exists := m.groups[group.ID]; !exists {
		return nil // behavior for not found in mocks usually varies, but this is safe
	}
