...
func (s *Storage) UpdateAgent(ctx context.Context, agent *types.Agent) error {
	labelsJSON, _ := json.Marshal(agent.Labels)
	capabilitiesJSON, _ := json.Marshal(agent.Capabilities)

	query := `
		UPDATE agents 
		SET name = ?, labels = ?, version = ?, capabilities = ?, updated_at = ?
		WHERE id = ?
	`
	...
	s.logger.Debug("Updated agent details", zap.String("agent_id", agent.ID.String()))
	return nil
}
...
func (s *Storage) CreateGroup(ctx context.Context, group *types.Group) error {
	...
	query := `
		INSERT INTO groups (id, name, labels, created_at, updated_at, otlp_http_endpoint, otlp_grpc_endpoint)
		VALUES (?, ?, ?, ?, ?, ?, ?)
	`
	...
	_, err := s.db.ExecContext(ctx, query,
		...
		group.CreatedAt,
		group.UpdatedAt,
		group.OTLPHTTPEndpoint,
		group.OTLPGRPCEndpoint,
	)
...
func (s *Storage) GetGroup(ctx context.Context, id string) (*types.Group, error) {
	query := `SELECT id, name, labels, created_at, updated_at, otlp_http_endpoint, otlp_grpc_endpoint FROM groups WHERE id = ?`

	var group types.Group
	var labelsJSON string
	var otlpHTTP, otlpGRPC sql.NullString
	...
	if otlpHTTP.Valid {
		group.OTLPHTTPEndpoint = &otlpHTTP.String
	}
	if otlpGRPC.Valid {
		group.OTLPGRPCEndpoint = &otlpGRPC.String
	}
...
func (s *Storage) ListGroups(ctx context.Context) ([]*types.Group, error) {
	query := `SELECT id, name, labels, created_at, updated_at, otlp_http_endpoint, otlp_grpc_endpoint FROM groups ORDER BY created_at DESC`
...
func (s *Storage) UpdateGroup(ctx context.Context, group *types.Group) error {
	...
	query := `
		UPDATE groups 
		SET name = ?, labels = ?, updated_at = ?, otlp_http_endpoint = ?, otlp_grpc_endpoint = ?
		WHERE id = ?
	`
	...
	s.logger.Debug("Updated group", zap.String("group_id", group.ID))
	return nil
}
