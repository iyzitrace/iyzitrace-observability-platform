...
func (s *Server) processAgentGrouping(ctx context.Context, agent *Agent, msg *protobufs.AgentToServer) {
	...
	// Fetch group details to get OTLP endpoints
	if groupID != "" {
		if group, err := s.agentService.GetGroup(ctx, groupID); err == nil && group != nil {
			agent.GroupOTLPHTTPEndpoint = group.OTLPHTTPEndpoint
			agent.GroupOTLPGRPCEndpoint = group.OTLPGRPCEndpoint
			s.logger.Debug("Updated agent group OTLP endpoints",
				zap.String("agentId", agent.InstanceIdStr),
				zap.String("groupId", groupID))
		}
	} else {
		// Clear group endpoints if not in a group
		agent.GroupOTLPHTTPEndpoint = nil
		agent.GroupOTLPGRPCEndpoint = nil
	}
	agent.mux.Unlock()
...
func (s *Server) persistAgent(ctx context.Context, agent *Agent, msg *protobufs.AgentToServer) {
	...
	// Use cached status if available, fallback to message
	agentStatus := agent.GetStatus()
	var desc *protobufs.AgentDescription
	var caps uint64

	if agentStatus != nil {
		desc = agentStatus.AgentDescription
		caps = agentStatus.Capabilities
	}
	...
	name := s.extractAgentName(desc)
	labels := s.extractAgentLabels(desc)
	version := s.extractAgentVersion(desc)
	capabilities := s.extractAgentCapabilities(caps)
	...
	if existingAgent == nil {
		...
		// Update existing agent details if we have new info
		shouldUpdateDetails := false
		if desc != nil {
			// Don't overwrite with "unknown" if we already have a name/version
			if name != "unknown" {
		...
		if shouldUpdateDetails {
			...
			if err := s.agentService.UpdateAgent(ctx, existingAgent); err != nil {
				s.logger.Error("Failed to update agent details",
					zap.String("agentId", agent.InstanceIdStr),
					zap.Error(err))
			}
		}
...
func (s *Server) calcConnectionSettings(agent *Agent, response *protobufs.ServerToAgent) {
	...
	// Priority: Group Override > Global Config
	var baseEndpoint string

	// Check for Group overrides first
	agent.mux.RLock()
	groupHTTP := agent.GroupOTLPHTTPEndpoint
	groupGRPC := agent.GroupOTLPGRPCEndpoint
	agent.mux.RUnlock()

	if groupHTTP != nil && *groupHTTP != "" {
		baseEndpoint = s.getOTLPEndpointForAgent(*groupHTTP)
	} else if groupGRPC != nil && *groupGRPC != "" {
		baseEndpoint = s.getOTLPEndpointForAgent(*groupGRPC)
	} else {
	...
	// Ensure baseEndpoint doesn't end with slash if we append /v1/...
	baseEndpoint = strings.TrimSuffix(baseEndpoint, "/")

	metricsURL := protocol + baseEndpoint + "/v1/metrics"
	tracesURL := protocol + baseEndpoint + "/v1/traces"
	logsURL := protocol + baseEndpoint + "/v1/logs"
