...
func (s *AgentServiceImpl) UpdateAgent(ctx context.Context, agent *Agent) error {
	storageAgent := &applicationstore.Agent{
		ID:           agent.ID,
		Name:         agent.Name,
		Labels:       agent.Labels,
	...
	return s.appStore.UpdateAgent(ctx, storageAgent)
}
...
func (s *AgentServiceImpl) CreateGroup(ctx context.Context, group *Group) error {
	storageGroup := &applicationstore.Group{
		...
		OTLPHTTPEndpoint: group.OTLPHTTPEndpoint,
		OTLPGRPCEndpoint: group.OTLPGRPCEndpoint,
	}
